...
object WebJars {
  ...
  private val prismLanguages = Seq("markup",
                                   "css",
                                   "clike",
                                   "javascript",
                                   "bash",
  ...
  object JS {
    ...
    val prismjsLangs =
      prismLanguages.map(lang => jsMin(s"prismjs/components/prism-$lang"))
    val prismjsExts = prismExtensions.map {
      case (ext, _) => jsMin(s"prismjs/plugins/$ext/prism-$ext")
    }
  }

  object CSS {
    val prismjs = css("prismjs/themes/prism-twilight")
    val prismjsExts = prismExtensions.filter(_._2).map {
      case (ext, _) => css(s"prismjs/plugins/$ext/prism-$ext")
    }
  }
