...
import hepek.utils.html.CodeUtils._
...
object Index extends ProgramiranjeTemplate {
  ...
  val uvodSectionContent = div(
    ...
    "Ovdje bi mogli početi recimo od Boole-ove algebre, tranzistora, struje i napona itd.", br,
    "Drugi pristup bi bio suprotan, odozgo-nadolje (top-down). Počinje se od generalnog pregleda onog o čemu se priča. ",
    "Koristićemo uglavnom drugi pristup. Zašto? Zato što sam ja ovdje glavni! :D", br,
    "Mislim da se premalo radi na ", b("približavanju materije učenicima"), ". Što će nama sve ovo, koja je svrha? Odakle da počnem? To su vrlo bitne stvari."
  )

  val komunikacijaSectionContent = div(
    p("""Mi ljudi za sebe tvrdimo da smo inteligentna bića.
...
Danas u svijetu postoji više od 5.000 jezika, od kojih većina nažalost lagano izumire."""),
    p("""Ljudi pričaju i sa robotima, ako ništa gledali ste futurističke filmove.
...
Neki od tih organa su predstavljeni kompjuterom, mehaničkim dijelovima, mikrofonom, zvučnikom itd."""),
    p("""Kompjuter je mozak mašine, on upravlja svime.
...
Te kompjuterske misli su ustvari naše naredbe, koje se skupno nazivaju program."""),
    p("""Ovdje smo da naučimo neke od načina za komuniciranje s kompjuterima.
...
"""),
    p("""Jedan od značajnijih problema koje p.j. pokušavaju riješiti je kompleksnost.
...
"""Dosta jezika danas je miks dvije ili više ovih paradigmi.""")
  ...
  val apstrakcijaSectionContent = div(
    p(  """
...
U matematici također imamo apstrakciju broja. Kada kažemo broj obično mislimo na cijeli broj. Ali ne kažemo KOJI broj!? :D Dakle, samo znamo da mislimo na NEKI, bilo koji broj.
        """),
        p("Većina ozbiljnih p.j. ima ", b("tipove"),". One koji nemaju slobodno izbjegavajte! ;)",
            
"Tip je apstrakcija koju dati p.j. razumije. Npr. kada napišemo ", javaSnippet("int broj", true) , 
""", ovo "int" je tip varijable "broj". Tako će kompjuter znati, između ostalog, """, i("koliko memorije da rezerviše"), """ za tu varijablu.
...
"""),
p("""Programski jezici sadrže apstrakcije za opisivanje komplikovanijih podataka, za tok programa itd.
...
""")
  ...
  val programiSectionContent = div(
    p(
      "Rekli smo da je program ", b("niz naredbi"), " koje računar treba izvršiti, u cilju dobijanja nekog rezultata.", br,
    ...
    p(
      ...
      ul(
        li(b("ulaz"), " - bio to naziv fajla, broj koji unese korisnik pomoću tastature, pokret miša, svejedno..."),
  ...
  val komunikacijaSection = Section("Komunikacija", komunikacijaSectionContent)
  val apstrakcijaSection = Section("Apstrakcija", apstrakcijaSectionContent)
  val programiSection = Section("Programi", programiSectionContent)

  override def sections = Seq(uvodSection, komunikacijaSection, apstrakcijaSection, programiSection)
}
